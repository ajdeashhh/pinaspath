...
st.sidebar.markdown(
    '<span class="mode-badge mode-train">Train</span> <span class="mode-badge mode-bus">Bus</span> <span class="mode-badge mode-jeepney">Jeepney</span> <span class="mode-badge mode-walk">Walk</span>',
    unsafe_allow_html=True,
)
...
def build_full_graph(stops_df, routes_df, add_walk_links=True, walk_thresh_m=700):
    ...
    for _, r in routes_df.iterrows():
        ...
        if G.has_edge(u, v):
            ...
            if not any(
                x.get("route_name") == rn and x.get("mode") == mode for x in routes_list
            ):
                routes_list.append({"route_name": rn, "mode": mode})
            existing["route_names"] = routes_list
        else:
            G.add_edge(
                u, v, travel_time=w, route_names=[{"route_name": rn, "mode": mode}]
            )
...
if isolated:
    st.sidebar.warning(
        f"{len(isolated)} isolated stop(s): {', '.join(isolated[:10])} (showing up to 10)."
    )
...
with left_col:
    if st.session_state.get("last_result") is None:
        ...
        if not valid_coords.empty and show_map:
            base_center = [
                valid_coords["lat"].astype(float).mean(),
                valid_coords["lon"].astype(float).mean(),
            ]
            base_map = folium.Map(
                location=base_center, zoom_start=12, tiles="CartoDB positron"
            )
            ...
            # draw legend box
            legend_html = """
            <div style="position: fixed; bottom: 50px; left: 10px; width:150px; height:110px; background-color: white; z-index:9999; font-size:12px; border-radius:8px; padding:8px; box-shadow:0 2px 6px rgba(0,0,0,0.15)">
            <b>Legend</b><br>
            ...
            st.info(
                "No valid lat/lon coordinates found in stops.csv — map disabled. Please add numeric lat,lon for stops or ensure stops.csv has no comment lines inside data rows."
            )
    else:
        # show route map
        res = st.session_state["last_result"]
        orig = res["path"][0]
        lat0 = G.nodes[orig]["lat"]
        lon0 = G.nodes[orig]["lon"]
        ...
        if lat0 is not None and lon0 is not None and show_map:
            ...
            for idx, leg in enumerate(res["legs"]):
                ...
                if "train" in mode:
                    ...
                    color = "#1f77b4"
                elif "jeep" in mode or "jeepney" in mode:
                    color = "#ff7f0e"
                elif "walk" in mode:
                    color = "#7f7f7f"
                ...
                if (
                    udata.get("lat") is not None
                    and udata.get("lon") is not None
                    and vdata.get("lat") is not None
                    and vdata.get("lon") is not None
                ):
                    folium.PolyLine(
            ...
            if last_nd.get("lat") is not None and last_nd.get("lon") is not None:
                folium.CircleMarker(
                    location=(last_nd["lat"], last_nd["lon"]),
                    radius=6,
                    color="#d62728",
                    fill=True,
